...
const BoardView3 = ({onBoardPg, boardSeq}) => {
    ...
    const onDelClick = () => {
        ...
        if(confirmation === boardDTO.password) {
            axios.delete(`http://localhost:8080/board/getView/${boardSeq}`)
                 .then(res => {
                     setBoardDTO(res.data)
                     alert('삭제 완')
            ...
            <div className={ Vstyles.view_main }>
                <div className={ Vstyles.top_div }>
                    ...
                    <h2 className={ Vstyles.view_title }>{boardDTO.title}</h2> 
                        <p>{boardDTO.writer} | {getToday(boardDTO.logTime)} | {boardDTO.hit}</p>
